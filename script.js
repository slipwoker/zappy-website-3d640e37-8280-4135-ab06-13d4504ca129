document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (isRequired && value === '') {
        setError(field, label.trim() + ' is required.');
        return false;
      }
      ...
      if (value !== '') {
        ...
        if (minLength && value.length < parseInt(minLength, 10)) {
          setError(field, label.trim() + ' must be at least ' + minLength + ' characters.');
          return false;
        }

        const maxLength = field.getAttribute('maxlength');
        if (maxLength && value.length > parseInt(maxLength, 10)) {
          setError(field, label.trim() + ' must not exceed ' + maxLength + ' characters.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      let successMsg = contactForm.querySelector('.form__success-message');
      if (!successMsg) {
        successMsg = document.createElement('p');
        successMsg.className = 'form__success-message';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מאוני טכנאי מזגנים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '3d640e37-8280-4135-ab06-13d4504ca129',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '3d640e37-8280-4135-ab06-13d4504ca129';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '3d640e37-8280-4135-ab06-13d4504ca129';
